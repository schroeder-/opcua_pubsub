...
use opcua_types::{Variant, DataValue, FieldMetaData};
use opcua_types::{AttributeId, DeadbandType, DateTime, LocalizedText};
...
impl PubSubFieldMetaData{
    ...
    pub fn new_from_var(var: &Variable, dt: &DataTypeId) -> Self {
        ...
        fmd.built_in_type = VariantTypeId::try_from(&fmd.data_type)
                                .unwrap_or(VariantTypeId::ExtensionObject).precedence();
...
impl PubSubFieldMetaDataBuilder{
    pub fn new() -> Self{
        PubSubFieldMetaDataBuilder{ data: create_empty_field_meta_data() }
    }
