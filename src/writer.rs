...
impl DataSetWriterBuilder{
    ...
    pub fn set_message_setting<'a>(&'a mut self, mask: UadpDataSetMessageContentFlags) -> &'a mut Self{
...
impl DataSetWriter{
    ...
    pub fn generate_header(&self, msg: & mut UadpDataSetMessage){
        ...
        if self.message_content_mask.contains(UadpDataSetMessageContentFlags::MAJORVERSION){
            msg.header.cfg_major_version = Some(self.config_version.major_version);
        }
        if self.message_content_mask.contains(UadpDataSetMessageContentFlags::MINORVERSION){
            msg.header.cfg_minor_version = Some(self.config_version.minor_version);
        }
        if self.message_content_mask.contains(UadpDataSetMessageContentFlags::PICOSECONDS){
            // Pico seconds are not supported
            msg.header.pico_seconds = Some(0);
        }
        if self.message_content_mask.contains(UadpDataSetMessageContentFlags::SEQUENCENUMBER){
            msg.header.sequence_no = Some(self.sequence_no);
        }
        if self.message_content_mask.contains(UadpDataSetMessageContentFlags::STATUS){
            //@TODO what todo with the status code?
            // 0 represents StatusCode Good
            msg.header.status = Some(0_u16);
        }
        if self.message_content_mask.contains(UadpDataSetMessageContentFlags::TIMESTAMP){
            msg.header.time_stamp =  Some(DateTime::now());
        }
    }
    pub fn generate_message(&mut self, ds: Vec::<(Promoted, DataValue)>,  config_version: &ConfigurationVersionDataType, publishing_interval: Duration) -> Option<UadpDataSetMessage>{
        ...
        let dataset = if ds.len() > 1 && self.delta_frame_counter < self.key_frame_count {
            let timediff = chrono::Duration::milliseconds((publishing_interval * self.delta_frame_counter as f64) as i64);
            ...
            else if self.field_content_mask.contains(DataSetFieldContentFlags::RAWDATA){
            ...
            // Generate a deltaframe with dataValue
                ...
                for (_, mut d) in ds{
                    ...
                    if let Some(dt) = &d.source_timestamp{
                        if dt.ticks() > offset.ticks(){
                            if !self.field_content_mask.contains(DataSetFieldContentFlags::SOURCETIMESTAMP){
                                d.server_timestamp = None;
                            }
                            if !self.field_content_mask.contains(DataSetFieldContentFlags::SERVERTIMESTAMP){
                                d.source_timestamp = None;
                            }
                            if !self.field_content_mask.contains(DataSetFieldContentFlags::STATUSCODE){
                                d.status = None;
                            }
                            vec.push((cnt, d));
                        }
            ...
            if self.field_content_mask.is_empty(){
                let mut vec = Vec::new();
                for (_, d) in ds{
                    let v = d.value.unwrap_or(Variant::StatusCode(d.status.unwrap_or(StatusCode::BadUnexpectedError)));
                    vec.push(v);
                }
...
impl WriterGroupe{
    ...
    pub fn tick(& mut self) -> bool{
        ...
        (self.last_action.ticks() + (self.publishing_interval * 1000.0) as i64) < DateTime::now().ticks()
    }

    pub fn generate_message<T: DataSetInfo>(& mut self, network_no: u16, publisher_id: &Variant, ds: & T) -> Option<UadpNetworkMessage>{
        //@TODO do keep alive
        let mut message = UadpNetworkMessage::new();
        if self.message_settings.contains(UadpNetworkMessageContentFlags::DATASETCLASSID){
            message.header.dataset_class_id = Some(Guid::null());
        }
        if self.message_settings.contains(UadpNetworkMessageContentFlags::PUBLISHERID){
        ...
        if self.message_settings.contains(UadpNetworkMessageContentFlags::GROUPHEADER){
            ...
            let net_no = if self.message_settings.contains(UadpNetworkMessageContentFlags::NETWORKMESSAGENUMBER){
                Some(network_no)
            } else {
                None
            };
            let seq_no = if self.message_settings.contains(UadpNetworkMessageContentFlags::SEQUENCENUMBER){
                Some(self.sequence_no)
            } else {
                None
            };
            let wg_id = if self.message_settings.contains(UadpNetworkMessageContentFlags::WRITERGROUPID){
        ...
        if self.message_settings.contains(UadpNetworkMessageContentFlags::PICOSECONDS){
            message.picoseconds = Some(0);
        }
        
        if self.message_settings.contains(UadpNetworkMessageContentFlags::TIMESTAMP){
        ...
        for w in self.writer.iter_mut(){
            ...
            if sz == 1{
                if self.message_settings.contains(UadpNetworkMessageContentFlags::PROMOTEDFIELDS){
            ...
            if let Some(ds) = dataset{
                ...
                if self.message_settings.contains(UadpNetworkMessageContentFlags::PAYLOADHEADER){
...
impl WriterGroupeBuilder{
    ...
    pub fn set_message_setting<'a>(&'a mut self, mask: UadpNetworkMessageContentFlags) -> &'a mut Self{
